...
func TestCommentsQueueTemplateExecutes(t *testing.T) {
	...
	data := map[string]any{
		"CSRFToken":   "csrf",
		"RequestPath": "/admin/comments/pending",
		"Status":      domain.CommentPending,
	...
	if err := tmpl.ExecuteTemplate(io.Discard, "page_detail.html", map[string]any{
		"CSRFToken":   "csrf",
		"RequestPath": "/admin/pages/1",
		"Page":        &domain.Page{ID: 1, PageKey: "/post", State: domain.PageOpen},
		"Comments":    []*domain.Comment{comment},
	}); err != nil {
		t.Fatalf("execute page_detail.html: %v", err)
	}
	if err := tmpl.ExecuteTemplate(io.Discard, "comment_detail.html", map[string]any{
		"CSRFToken":   "csrf",
		"RequestPath": "/admin/comments/comment-1",
		"Comment":     comment,
		"Events":      []domain.ModerationEvent{},
	}); err != nil {
		t.Fatalf("execute comment_detail.html: %v", err)
	}
...
func mustAdminTemplate(t *testing.T) *template.Template {
	...
	funcs := template.FuncMap{
		...
		"navActive":    func(current, item string) bool { return current == item },
